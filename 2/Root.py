...
from param import width, height
from star import Star

def draw_grid(canvas):
	for i in range(0, width, 20):
		canvas.create_line(i, 0, i, width, fill='black')
	for i in range(0, height, 20):
		canvas.create_line(0, i, height, i, fill='black')
...
class Window:
	def __init__(self):
		self.root = tk.Tk()
		self.root.title('2')
		# Создаем канвас с белым фоном
		self.canvas = tk.Canvas(self.root, width=width, height=height, bg='darkgrey')
